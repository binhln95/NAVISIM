...
/*//    LAM*/
    const auto vid = atoi(vidpid.substr(0, splitPos).c_str());
    const auto pid = atoi(vidpid.substr(splitPos+1).c_str());
...
//    for(int i=0; i<usbDeviceCount; ++i){
        /*//        LAM*/
        if(libusb_open2(devs, &dev_handle,limeFd) != 0)
    ...
    libusb_free_config_descriptor(descriptor);*///LAM

    bulkCtrlAvailable = true;//LAM
    isConnected = true;
