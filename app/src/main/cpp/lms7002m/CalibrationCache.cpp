...
"VALUES ( " << boardId << "," << llrint(frequency) << "," << (int)channel << "," << (transmitter?1:0) << "," <<vco<<","<<csw<<");";
...
auto lambda_callback = [](void *vco_csw_pair, int argc, char **argv, char **azColName)
{
    ...
    if(vco_csw != nullptr)
    {
        vco_csw->vco = argv[0] != nullptr ? atoi(argv[0]) : 0;
        vco_csw->csw = argv[1] != nullptr ? atoi(argv[1]) : 128;
        vco_csw->found = true;
        return 0;
    }
    return 1;
...
"boardID="<<boardId<<
" AND frequency="<<llrint(frequency)<<
...
"VALUES ( " << boardId << "," << llrint(frequency) << "," << (int)channel << "," << (transmitter?1:0) << "," << band_lna << ", " <<
...
int CalibrationCache::GetDC_IQ(uint32_t boardId, double frequency, uint8_t channel, bool transmitter, int band_lna, int *dcI, int *dcQ, int *gainI, int *gainQ, int *phaseOffset)
{
    ...
    auto lambda_callback = [](void *dc_iq_data, int argc, char **argv, char **azColName)
    {
        ...
        if(data != nullptr)
        {
            data->dcI = argv[0] != nullptr ? atoi(argv[0]) : 0;
            data->dcQ = argv[1] != nullptr ? atoi(argv[1]) : 0;
            data->gainI = argv[2] != nullptr ? atoi(argv[2]) : 0;
            data->gainQ = argv[3] != nullptr ? atoi(argv[3]) : 0;
            data->phaseOffset = argv[4] != nullptr ? atoi(argv[4]) : 0;
            data->found = true;
            return 0;
        }
        return 1;
...
"boardID="<<boardId<<
" AND frequency >= "<<llrint(frequency)<<
" AND frequency < "<<llrint(frequency + 1e6)<<
" AND channel="<<(int)channel<<
" AND transmitter="<<(transmitter?1:0)<<
" AND band_lna="<<band_lna<<
...
";";
    ...
    *dcI = rint(linearInterp(frequency, f0, dcI0, f1, dcI1));
    *dcQ = rint(linearInterp(frequency, f0, dcQ0, f1, dcQ1));
    *gainI = rint(linearInterp(frequency, f0, gainI0, f1, gainI1));
    *gainQ = rint(linearInterp(frequency, f0, gainQ0, f1, gainQ1));
    *phaseOffset = rint(linearInterp(frequency, f0, phaseOffset0, f1, phaseOffset1));
...
"VALUES ( " << boardId << "," << llrint(bandwidth) << "," << (int)channel << "," << (transmitter?1:0) << "," << filter_id << ", " <<
rcal<<","<<ccal<<","<<cfb<<");";
...
int CalibrationCache::GetFilter_RC(uint32_t boardId, double bandwidth, uint8_t channel, bool transmitter, int filter_id, int *rcal, int *ccal, int *cfb)
{
    ...
    auto lambda_callback = [](void *filter_rc_data, int argc, char **argv, char **azColName)
    {
        ...
        if(data != nullptr)
        {
            data->rcal = argv[0] != nullptr ? atoi(argv[0]) : 0;
            data->ccal = argv[1] != nullptr ? atoi(argv[1]) : 0;
            data->cfb = argv[2] != nullptr ? atoi(argv[2]) : 0;
            data->found = true;
            return 0;
        }
        return 1;
...
"boardID="<<boardId<<
" AND bandwidth="<<llrint(bandwidth)<<
" AND channel="<<(int)channel<<
" AND transmitter="<<(transmitter?1:0)<<
