...
int LMS7002M::SetTxDCOffset(const float_type I, const float_type Q)
{
    ...
    this->Modify_SPI_Reg_bits(LMS7param(DCCORRI_TXTSP), lrint(I*128));
    this->Modify_SPI_Reg_bits(LMS7param(DCCORRQ_TXTSP), lrint(Q*128));
	return 0;
}
...
int LMS7002M::SetIQBalance(const bool tx, const float_type phase, const float_type gainI, const float_type gainQ)
{
    ...
    int iqcorr = lrint(2047*(phase/(M_PI/2)));
    int gcorri = lrint(2047*gainI);
    int gcorrq = lrint(2047*gainQ);
