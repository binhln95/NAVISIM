...
const char* externalStoragePath;

double xyzWayPoint[USER_MOTION_SIZE][3];
double durationWayPoint[USER_MOTION_SIZE];
int nWayPoint = 0;
...
int readUserMotionInLLH(double **xyz, const char *filename)
{
    FILE *fp;
    int numd;
    char str[MAX_CHAR];
    ...
    if (NULL == (fp = fopen(filename, "rt")))
        return(-1);
    if (fgets(str, MAX_CHAR, fp) != NULL)//Skip first line
        for (numd = 0; numd<WAY_POINT_SIZE; numd++)
        {
            if (fgets(str, MAX_CHAR, fp) == NULL)
                break;

            if (EOF == sscanf(str, "%lf %lf %lf %lf", &x, &y, &z, &t)) // Read CSV line
                break;
            ...
            llh[1] = y / R2D;
            llh[2] = z / R2D;
            llh2xyz(llh, xyzWayPoint[numd]);
            durationWayPoint[numd] = t*60;
        }
    nWayPoint = numd;
    fclose(fp);


    return durationWayPoint[numd-1]*10;
...
void setCpuPriority (int indexCpu, int priority){
    ...
    strcpy(command,"su -c \"echo 190000 > /sys/devices/system/cpu/cpu");
    sprintf(cpuNum,"%d",indexCpu);
    strcat(command,cpuNum);
    strcat(command,"/cpufreq/scaling_min_freq\"");
    r = system(command);
    if (r != 0) {
        LOGE("Could not set cpu speed, error: %d\n", errno);
    }

    strcpy(command,"su -c \"echo performance > /sys/devices/system/cpu/cpu");
    sprintf(cpuNum,"%d",indexCpu);
    strcat(command,cpuNum);
    strcat(command,"/cpufreq/scaling_governor\"");
    r = system(command);
    if (r != 0) {
        LOGD("Could not set cpu governor, error: %d\n", errno);
    }


    pid = gettid();
    CPU_ZERO(&cpuSet);
    CPU_SET(indexCpu, &cpuSet);
    ret = sched_setaffinity(pid,sizeof(cpu_set_t),&cpuSet);
    if(ret !=0){
        LOGD("Unsuccessful in setting thread affinity, ret: %d, err: %d",ret,errno);
    }
    ...
    if (ret != 0)
    {
        LOGD("Unsuccessful in setting process priority ret: %d, err:%d ",ret, errno);
    }

    /*this_thread = pthread_self();
    params.__sched_priority = 10;
    ret = pthread_setschedparam(this_thread, SCHED_RR, &params);
    if (ret != 0)
    {
        LOGE("Unsuccessful in setting thread priority %d, ret: %d, err:%d ",params.sched_priority,ret, errno);
    }*/
}

void threadChannelElabFunc (thread_channel *cThread){
    setCpuPriority(MAX_CORE-(cThread->indexThread+1),-20);
    ...
    int ip,qp;
    while(isRunning) {
        pthread_mutex_lock(&(cThread->startSyncMutex));
        pthread_mutex_lock(&(cThread->runningMutex));
        isRunning = cThread->isRunning;
        pthread_mutex_unlock(&(cThread->runningMutex));
        int isamp = 0;
        int iLoop = 0;
        t_start = clock();
        double delt = cThread->channel[i].f_code /cThread->sampling_freq;

        for (isamp = 0; isamp<cThread->bufferSize; isamp++) {


            int i_acc = 0;
            int q_acc = 0;

            for (i = 0; i < MAX_CHANNEL_PER_THREAD; i++) {
                if (cThread->channel[i].prn > 0) {
                    iTable = (cThread->channel[i].carr_phase >> 16) & 511;

                    ip = cThread->channel[i].dataBit * cThread->channel[i].codeCA *
                         cosTable512[iTable] * cThread->channel[i].gainValue;
                    qp = cThread->channel[i].dataBit * cThread->channel[i].codeCA *
                         sinTable512[iTable] * cThread->channel[i].gainValue;
                    ...
                    // Update code phase
                    cThread->channel[i].code_phase += delt;

                    if (cThread->channel[i].code_phase >= CA_SEQ_LEN) {
                        cThread->channel[i].code_phase -= CA_SEQ_LEN;

                        cThread->channel[i].icode++;

                        if (cThread->channel[i].icode >= 20) // 20 C/A codes = 1 navigation data bit
                        {
                            cThread->channel[i].icode = 0;
                            cThread->channel[i].ibit++;

                            if (cThread->channel[i].ibit >= 30) // 30 navigation data bits = 1 word
                            {
                                cThread->channel[i].ibit = 0;
                                cThread->channel[i].iword++;
...
//                                printf("\nWARNING: Subframe word buffer overflow.\n");*/
                            ...
                            cThread->channel[i].dataBit =
                                    (int) ((cThread->channel[i].dwrd[cThread->channel[i].iword]
                                            >> (29 - cThread->channel[i].ibit)) & 0x1UL) * 2 - 1;
                        }
                    }

                    // Set currnt code chip
                    cThread->channel[i].codeCA =
                            cThread->channel[i].ca[(int) cThread->channel[i].code_phase] * 2 - 1;

                    // Update carrier phase
                    cThread->channel[i].carr_phase += cThread->channel[i].carr_phasestep;
                }
            }

            cThread->iAccBuff[isamp] = i_acc;
            cThread->qAccBuff[isamp] = q_acc;
        }
        pthread_mutex_unlock(&(cThread->stopSyncMutex));
        t_stop = clock();
        LOGD("Thread %d process time = %.1f [sec]; number of ms: %d\n", cThread->indexThread,
             (double) (t_stop - t_start) / CLOCKS_PER_SEC, iLoop);
    }


    LOGD("Channel thread %d finished",cThread->indexThread);
}
...
double runCore(char *sExternalStoragePath, int flag, int time)
{
    ...
    // Default options
    navfile[0] = 0;
    strcpy(navfile, sExternalStoragePath);
    strcat(navfile, "/brdc3540.14n");
    umfile[0] = 0;
    strcpy(umfile, sExternalStoragePath);
//	strcat(umfile, "/circle.csv");
    strcpy(outfile, sExternalStoragePath);
    strcat(outfile, "/gpssim.bin");
    samp_freq = TX_SAMPLERATE;
    data_format = SC16;
    g0.week = -1; // Invalid start time
    iduration = USER_MOTION_SIZE;
    duration = (double)iduration/500.0; // Default duration
    verb = FALSE;
    ionoutc.enable = TRUE;
    if (flag == 0) {
        strcpy(umfile, sExternalStoragePath);
        ...
        sscanf(sExternalStoragePath,"%lf,%lf,%lf",&xyz[0][0],&xyz[0][1],&xyz[0][2]);
    } else {
        return 0.0;
    }
    ...
    if (navfile[0]==0)
    {
        LOGE("ERROR: GPS ephemeris file is not specified.\n");
        exit(1);
    }
    ...
    if (!staticLocationMode)
    {
        ...
        if (numd==-1)
        {
            LOGE("ERROR: Failed to open user motion / NMEA GGA file.\n");
            exit(1);
        }
        else if (numd==0)
        {
            LOGE("ERROR: Failed to read user motion / NMEA GGA data.\n");
            exit(1);
        }
    ...
    else
    {
        ...
        LOGE("Using static location mode.\n");

        numd = iduration;
    }
/*
	LOGE("xyz = %11.1f, %11.1f, %11.1f\n", xyz[0][0], xyz[0][1], xyz[0][2]);
	LOGE("llh = %11.6f, %11.6f, %11.1f\n", llh[0]*R2D, llh[1]*R2D, llh[2]);
*/
    ////////////////////////////////////////////////////////////
    // Read ephemeris
    ////////////////////////////////////////////////////////////
    ...
    if (g0.week>=0) // Scenario start time has been set.
    {
        ...
        else
        {
            ...
            {
                LOGE("ERROR: Invalid start time.\n");
                LOGE("tmin = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)",
                     tmin.y, tmin.m, tmin.d, tmin.hh, tmin.mm, tmin.sec,
                     gmin.week, gmin.sec);
                LOGE("tmax = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)",
    ...
    LOGE("Start time = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n",
         t0.y, t0.m, t0.d, t0.hh, t0.mm, t0.sec, g0.week, g0.sec);
    LOGE("Duration = %.1f [sec]\n", ((double)numd)/10.0);
    ...
    if (ieph == -1)
    {
        LOGE("ERROR: No current set of ephemerides has been found.\n");
        exit(1);
    }
    ...
    if (iq_buff==NULL)
    {
        LOGE("ERROR: Faild to allocate 16-bit I/Q buffer.\n");
        exit(1);
    }
    ...
    if (data_format==SC08)
    {
        ...
        if (iq8_buff==NULL)
        {
            LOGE("ERROR: Faild to allocate 8-bit I/Q buffer.\n");
            exit(1);
        }
    }
    else if (data_format==SC01)
    {
        ...
        if (iq8_buff==NULL)
        {
            LOGE("ERROR: Faild to allocate compressed 1-bit I/Q buffer.\n");
            exit(1);
        }
    }
    ...
    for (iumd=1; iumd<numd; iumd++)
    {
        ...
        // Update time counter
        LOGE("\rTime into run = %4.1f", subGpsTime(grx, g0));
        fflush(stdout);
    }
...
int runCore2(char *sExternalStoragePath)
{
    ...
    int samplesPerCa;
    int iThread;
    int numThread = floor(MAX_CHAN/MAX_CHANNEL_PER_THREAD);
    thread_channel threadChannel[NUMBER_OF_THREAD];
    pthread_t threadChannelElaboration [NUMBER_OF_THREAD];
    ...
    if (navfile[0]==0)
    {
        LOGD( "ERROR: GPS ephemeris file is not specified.\n");
        exit(1);
    }
    ...
    delt = 1.0/samp_freq;
    samplesPerCa = floor((samp_freq/CODE_FREQ)*CA_SEQ_LEN);
    delt_1ms = delt*samplesPerCa;
//    samplesPerCa = 512;
    ...
    if (!staticLocationMode)
    {
        ...
        if (numd==-1)
        {
            LOGD( "ERROR: Failed to open user motion / NMEA GGA file.\n");
//            exit(1);
        }
        else if (numd==0)
        {
            LOGD( "ERROR: Failed to read user motion / NMEA GGA data.\n");
//            exit(1);
        }
    ...
    else
    {
        // Static geodetic coordinates input mode: "-l"
        // Added by scateu@gmail.com
        LOGD( "Using static location mode.\n");
        llh[0] = 35.681298 / R2D;
        llh[1] = 139.766247 / R2D;
        llh[2] = 10.0;
        llh2xyz(llh,xyz[0]);
    ...
    if (neph==0)
    {
        LOGD( "ERROR: No ephemeris available.\n");
//        exit(1);
    }

    if ((verb==TRUE)&&(ionoutc.vflg==TRUE))
    {
        LOGD( "  %12.3e %12.3e %12.3e %12.3e\n",
              ionoutc.alpha0, ionoutc.alpha1, ionoutc.alpha2, ionoutc.alpha3);
        LOGD( "  %12.3e %12.3e %12.3e %12.3e\n",
              ionoutc.beta0, ionoutc.beta1, ionoutc.beta2, ionoutc.beta3);
        LOGD( "   %19.11e %19.11e  %9d %9d\n",
              ionoutc.A0, ionoutc.A1, ionoutc.tot, ionoutc.wnt);
        LOGD( "%6d\n", ionoutc.dtls);
    }
    ...
    if (g0.week>=0) // Scenario start time has been set.
    {
        ...
        else
        {
            if (subGpsTime(g0, gmin)<0.0 || subGpsTime(gmax, g0)<0.0)
            {
                LOGD( "ERROR: Invalid start time.\n");
                LOGD( "tmin = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n",
                      tmin.y, tmin.m, tmin.d, tmin.hh, tmin.mm, tmin.sec,
                      gmin.week, gmin.sec);
                LOGD( "tmax = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n",
                      tmax.y, tmax.m, tmax.d, tmax.hh, tmax.mm, tmax.sec,
                      gmax.week, gmax.sec);
//                exit(1);
            }
    ...
    LOGD( "Start time = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n",
          t0.y, t0.m, t0.d, t0.hh, t0.mm, t0.sec, g0.week, g0.sec);
    LOGD( "Duration = %.1f [sec]\n", ((double)numd)/10.0);
    ...
    if (ieph == -1)
    {
        LOGD( "ERROR: No current set of ephemerides has been found.\n");
//        exit(1);
    }
    ...
    if (iq_buff==NULL)
    {
        LOGD( "ERROR: Faild to allocate 16-bit I/Q buffer.\n");
//        exit(1);
    }
    ...
    if (data_format==SC08)
    {
        ...
        if (iq8_buff==NULL)
        {
            LOGD( "ERROR: Faild to allocate 8-bit I/Q buffer.\n");
//            exit(1);
        }
    }
    else if (data_format==SC01)
    {
        ...
        if (iq8_buff==NULL)
        {
            LOGD( "ERROR: Faild to allocate compressed 1-bit I/Q buffer.\n");
//            exit(1);
        }
    }
    ...
    if(strcmp("-", outfile)){
        if (NULL==(fp=fopen(outfile,"wb")))
        {
            LOGD( "ERROR: Failed to open output file.\n");
            return 1;
//            exit(1);
        }
    }else{
...
//     Clear all channels
    for (i=0; i<MAX_CHAN; i++) {
        chan[i].prn = 0;
        chan[i].samplePerCA = samplesPerCa;
    }
    ...
    // Update receiver time
    grx = incGpsTime(grx, 0.1);
    int i_accArr[iq_buff_size] __attribute__ ((aligned(16)));
    int q_accArr[iq_buff_size] __attribute__ ((aligned(16)));

    for(iThread=0; iThread< NUMBER_OF_THREAD;iThread++){
        pthread_mutex_init(&(threadChannel[iThread].runningMutex), NULL);
        pthread_mutex_init(&(threadChannel[iThread].startSyncMutex), NULL);
        pthread_mutex_init(&(threadChannel[iThread].stopSyncMutex), NULL);
        pthread_cond_init(&threadChannel[iThread].readyToAdd,NULL);
        threadChannel[iThread].isRunning = 1;
        threadChannel[iThread].indexThread = iThread;
        threadChannel[iThread].bufferSize = iq_buff_size;
        threadChannel[iThread].sampling_freq = samp_freq;
        /* for(int ii; ii<MAX_CHANNEL_PER_THREAD; ii++){
             (threadChannel[iThread].channel[ii]) = &(chan[iThread*MAX_CHANNEL_PER_THREAD+ii]);
         }*/

        (threadChannel[iThread].channel) = &(chan[iThread*MAX_CHANNEL_PER_THREAD]);
        posix_memalign((void **)&(threadChannel[iThread].iAccBuff),POSIX_ALIGNMENT,iq_buff_size*sizeof(int));
        posix_memalign((void **)&(threadChannel[iThread].qAccBuff),POSIX_ALIGNMENT,iq_buff_size*sizeof(int));

        pthread_mutex_lock(&(threadChannel[iThread].startSyncMutex));
        pthread_mutex_lock(&(threadChannel[iThread].stopSyncMutex));
        pthread_create(&threadChannelElaboration[iThread],NULL,(void*)threadChannelElabFunc,(void*) &threadChannel[iThread]);
    }
    ...
    for (iumd=1; iumd<numd; iumd++){
        for (i=0; i<MAX_CHAN; i++){
            if (chan[i].prn>0){
                ...
                computeCodePhase(&chan[i], rho, 0.1);
#ifndef FLOAT_CARR_PHASE
                chan[i].carr_phasestep = (int)round(33554432.0 * chan[i].f_carr * delt); //  512.0 * 65536.0
#endif
                // Path loss
                path_loss = 20200000.0/rho.d;
        ...
        for(iThread=0;iThread<NUMBER_OF_THREAD;iThread++){
            pthread_mutex_unlock(&(threadChannel[iThread].startSyncMutex));
        }

        if(iumd>1){
            for(isamp=0;isamp<iq_buff_size;isamp++)
            {
                iq_buff[(isamp*2)]     = (short)((i_accArr[isamp] +64)>>7);
                iq_buff[(isamp*2)+1]   = (short)((q_accArr[isamp] +64)>>7);
            }
            fwrite(iq_buff, 2, (iq_buff_size*2), fp);
        }
        memset(i_accArr,0,iq_buff_size*sizeof(int));
        memset(q_accArr,0,iq_buff_size*sizeof(int));

        for(iThread=0;iThread<NUMBER_OF_THREAD;iThread++){
            pthread_mutex_lock(&(threadChannel[iThread].stopSyncMutex));
//			ArrayAddInt(i_accArr,threadChannel[iThread].iAccBuff,i_accArr,iq_buff_size);
//			ArrayAddInt(q_accArr,threadChannel[iThread].qAccBuff,q_accArr,iq_buff_size);
        }
        for(iThread=0;iThread<NUMBER_OF_THREAD;iThread++){
            for(i=0; i<iq_buff_size;i++){
                i_accArr[i] += threadChannel[iThread].iAccBuff[i];
                q_accArr[i] += threadChannel[iThread].qAccBuff[i];
            }
        }
        ...
        // Update time counter
        LOGD( "\rTime into run = %4.1f", subGpsTime(grx, g0));
        fflush(stdout);
    }
    ...
    LOGD( "\nDone!\n");
    for(iThread=0; iThread< NUMBER_OF_THREAD;iThread++){
        pthread_mutex_lock(&(threadChannel[iThread].runningMutex));
        threadChannel[iThread].isRunning = 0;
        pthread_mutex_unlock(&(threadChannel[iThread].runningMutex));

        pthread_mutex_unlock(&(threadChannel[iThread].startSyncMutex));
        pthread_mutex_unlock(&(threadChannel[iThread].stopSyncMutex));
    }

    for(iThread=0; iThread< NUMBER_OF_THREAD;iThread++){
        pthread_join(&(threadChannelElaboration[iThread]),NULL);
    }

    for(iThread=0; iThread< NUMBER_OF_THREAD;iThread++){

        pthread_mutex_destroy(&(threadChannel[iThread].startSyncMutex));
        pthread_mutex_destroy(&(threadChannel[iThread].stopSyncMutex));

        pthread_cond_destroy(&threadChannel[iThread].readyToAdd);
        free(threadChannel[iThread].iAccBuff);
        free(threadChannel[iThread].qAccBuff);
    ...
    // Process time
    LOGD( "Process time = %.1f [sec]\n", (double)(tend-tstart)/CLOCKS_PER_SEC);
    char command[256];
    sprintf(command,"su -c \"echo userspace > /sys/devices/system/cpu/cpu%d/cpufreq/scaling_governor\"",MAX_CORE-1);
    int r = system(command);
    if(r!=0) {
        LOGD("Could not set back cpu governor, r: %d\n",r);
    }
    for(iThread=0; iThread< NUMBER_OF_THREAD;iThread++){
        sprintf(command,"su -c \"echo userspace > /sys/devices/system/cpu/cpu%d/cpufreq/scaling_governor\"",MAX_CORE-(iThread+1));
        int r = system(command);
        if(r!=0) {
            LOGD("Could not set back cpu governor, r: %d\n",r);
        }
    ...
    return((int)(tend-tstart)/CLOCKS_PER_SEC);
}
...
void runCore_realTime(void *arg)
{
    ...
    datetime_t t0= getCurrentTime();
    date2gps(&t0, &s->opt.g0);
    //s->opt.g0=incGpsTime(s->opt.g0, timeOffset);
    ...
    // Default options
    navfile[0] = 0;
    strcpy(navfile, externalStoragePath);
    strcat(navfile, "/brdc3540.14n");
    umfile[0] = 0;
    strcpy(umfile, externalStoragePath);
//    strcat(umfile, "/circle.csv");
    strcat(umfile, "/Navistar.txt");
    strcpy(outfile, externalStoragePath);
    strcat(outfile, "/gpssim.bin");
    samp_freq = TX_SAMPLERATE;
    data_format = SC16;
    g0.week = -1; // Invalid start time
...
//            numd = readUserMotion(xyz, umfile);
            numd = readUserMotionInLLH(xyz,umfile);
...
//    neph = readRinexNavAll(eph, &ionoutc, navfile);
    ...
    for (iumd=1; iumd<numd; iumd++)
    {
        for ( iWP= 1; iWP < nWayPoint; iWP++)
        {
            durationWayPoint[iWP] = durationWayPoint[iWP];//Convert into second
        }
        double dTime = 0.1*iumd;

        for (iLine = 0; iLine < nWayPoint - 1; iLine++)
        {
            if (dTime >= durationWayPoint[iLine] && dTime < durationWayPoint[iLine + 1])
            {
                xyz[iumd][0] = xyzWayPoint[iLine][0] + (xyzWayPoint[iLine + 1][0] - xyzWayPoint[iLine][0])*(dTime - durationWayPoint[iLine]) / (durationWayPoint[iLine + 1] - durationWayPoint[iLine]);
                xyz[iumd][1] = xyzWayPoint[iLine][1] + (xyzWayPoint[iLine + 1][1] - xyzWayPoint[iLine][1])*(dTime - durationWayPoint[iLine]) / (durationWayPoint[iLine + 1] - durationWayPoint[iLine]);
                xyz[iumd][2] = xyzWayPoint[iLine][2] + (xyzWayPoint[iLine + 1][2] - xyzWayPoint[iLine][2])*(dTime - durationWayPoint[iLine]) / (durationWayPoint[iLine + 1] - durationWayPoint[iLine]);
                break;
            }
        }
...
//    (*env)->CallVoidMethod(env, mainActivityObj, timerId, javaMsg);
    ...
    strcpy(navfile, traj);
    umfile[0] = 0;
//    strcpy(umfile, "/storage/emulated/0/binhln/circle.csv");
    strcpy(outfile, "/storage/emulated/0/dataNAVISIM/gpssim.bin");
    samp_freq = 2.0e6;
    data_format = SC16;
    g0.week = -1; // Invalid start time
    iduration = USER_MOTION_SIZE;
    duration = (double)iduration/6000.0; // Default duration
    verb = FALSE;
    ionoutc.enable = TRUE;
    if (flag == 0) {
        strcpy(umfile, path);
        ...
        staticLocationMode = TRUE;
        sscanf(path,"%lf,%lf,%lf",&xyz[0][0],&xyz[0][1],&xyz[0][2]);
    } else {
        return 0.0;
    }
