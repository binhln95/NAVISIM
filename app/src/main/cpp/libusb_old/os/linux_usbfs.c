...
static int check_usb_vfs(const char *dirname)
{
	...
	if(r!=0) {
		LOGE("Could not grant permissions to USB, r: %d\n",r);
	}*/
	dir = opendir(dirname);
	if (!dir) {
		LOGE("-----linux_usbfs.c-----check_usb_vfs----Cannot open directory %s\n",dirname);
		return 0;
	}
	...
	while ((entry = readdir(dir)) != NULL) {
		if (entry->d_name[0] == '.')
			continue;
		LOGD("linux----check_usb_vfs-----have file in right place");
...
static int op_init(struct libusb_context *ctx)
{
	...
	if (!usbfs_path) {
		...
		LOGE("USBI init: Could not find usbfs path \n");
		return LIBUSB_ERROR_OTHER;
	}
#else
	usbfs_path = usbDevPath;
#endif
	...
	if (supports_flag_bulk_continuation == -1) {
		...
		if (supports_flag_bulk_continuation == -1) {
			...
			LOGE("USBI init: error checking for bulk continuation support \n");
			return LIBUSB_ERROR_OTHER;
		}
	}
	...
	if (-1 == supports_flag_zero_packet) {
		...
		if (-1 == supports_flag_zero_packet) {
			...
			LOGE("USBI init: error checking for zero length packet support \n");
			return LIBUSB_ERROR_OTHER;
		}
	}
	...
	struct discovered_devs **_discdevs, uint8_t busnum)
{
	...
	usbi_dbg("%s", dirpath);
	char command[256];
	strcpy(command, "su -c \"chmod -R 777 ");
	strcat(command,usbfs_path);
	strcat(command,"\"");
	r = system(command);
	dir = opendir(dirpath);
	if (!dir) {
...
static int find_fd_by_name(char *file_name)
{
	...
	DIR *proc_fd = opendir("/proc/bus/usb");
	int ret = -1;
	...
	while (fd_dirent = readdir(proc_fd))
	{
		...
		snprintf(link_file_name, 1024, "/proc/bus/usb/%s", fd_dirent->d_name);

		memset(fd_file_name, 0, sizeof(fd_file_name));
		readlink(link_file_name, fd_file_name, sizeof(fd_file_name) - 1);
		//LOGD("~~~~~~~~~~find_fd_by_name~~~~~~fd_file_name: %s \t filenam:%s",fd_file_name,file_name);
...
static int _get_usbfs_fd(struct libusb_device *dev, mode_t mode, int silent)
{
	...
	//int r = system("su -c \"chmod -R 777 /sys/\"");
	if(r!=0) {
		LOGE("Could not grant permissions to USB, r: %d\n",r);
	}
	fd = open(path, mode);
	LOGD("find fd func-------path: %s; fd: %d",path,fd);
	if (fd != -1)
		return fd; /* Success */

	if (errno == ENOENT) {
		if (!silent)
			LOGE("File doesn't exist, wait %d ms and try again", delay/1000);
...
//	hpriv->fd = find_fd_by_name(filename);
	...
	LOGD("fd from C: %d; filename: %s",hpriv->fd, filename);
// *** PrimeSense patch for Android ***

//	hpriv->fd = _get_usbfs_fd(handle->dev, O_RDWR, 0);
    hpriv->fd = limeFd;
	LOGD("fd from C: %d",hpriv->fd);
...
static int op_set_configuration(struct libusb_device_handle *handle, int config)
{
	...
	if (r) {
		LOGE("----Linux_usbfs----set-configuration---------: fd: %d, r:%d",fd,r);
		if (errno == EINVAL)
			return LIBUSB_ERROR_NOT_FOUND;
		else if (errno == EBUSY)
...
static int op_reset_device(struct libusb_device_handle *handle)
{
	...
	if (r) {
		if (errno == ENODEV) {
			...
			LOGE("RESET ERR: LIBUSB_ERROR_NOT_FOUND");
			goto out;
		}
		...
		LOGE("RESET ERR: LIBUSB_ERROR_OTHER");
		goto out;
	}
...
static int submit_control_transfer(struct usbi_transfer *itransfer)
{
	...
	if (r < 0) {
		...
		usbi_err(TRANSFER_CTX(transfer),
			"submiturb failed error %d errno=%d", r, errno);
		LOGE("submiturb failed error %d errno=%d  fd: %d", r, errno,dpriv->fd);
		return LIBUSB_ERROR_IO;
	}
	return 0;
}
	...
	struct usbfs_urb *urb)
{
	...
	if (tpriv->reap_action != NORMAL) {
		...
		if (tpriv->num_retired == tpriv->num_urbs) {
			LOGD(" handle_bulk_completion - abnormal reap: last URB handled, reporting");
	...
	case -ECOMM:
	case -ENOSR:
		LOGD("handle_bulk_completion - low level error %d", urb->status);
		tpriv->reap_action = ERROR;
		goto cancel_remaining;
	default:
		LOGD("handle_bulk_completion - unrecognised urb status %d", urb->status);
		tpriv->reap_action = ERROR;
		goto cancel_remaining;
	}
	...
	struct usbfs_urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		case -ENOSR:
		case -EXDEV:
			LOGD("handle_iso_completion - low-level USB error %d", urb_desc->status);
			lib_desc->status = LIBUSB_TRANSFER_ERROR;
			break;
		default:
			LOGD("handle_iso_completion - unrecognised urb status %d", urb_desc->status);
			lib_desc->status = LIBUSB_TRANSFER_ERROR;
			break;
		}
	...
	if (tpriv->reap_action != NORMAL) { /* cancelled or submit_fail */
		LOGD("handle_iso_completion - CANCEL: urb status %d", urb->status);

		if (tpriv->num_retired == num_urbs) {
			LOGD("handle_iso_completion - CANCEL: last URB handled, reporting");
			free_iso_urbs(tpriv);
	...
	case -ESHUTDOWN:
		...
		break;
	default:
		LOGD("unrecognised urb status %d", urb->status);
	...
	struct usbfs_urb *urb)
{
	...
	case -ECOMM:
	case -ENOSR:
		LOGE("handle_control_completion - low-level bus error occurred");
		status = LIBUSB_TRANSFER_ERROR;
		break;
	default:
		LOGD("handle_control_completion - unrecognised urb status %d", urb->status);
		status = LIBUSB_TRANSFER_ERROR;
		break;
	}
...
//	LOGD("-----linux.c------reap_for_handle----urb type=%d status=%d urb length=%d", urb->type, urb->status,urb->actual_length);
	switch (transfer->type) {
	case LIBUSB_TRANSFER_TYPE_ISOCHRONOUS:
	...
	struct pollfd *fds, POLL_NFDS_TYPE nfds, int num_ready)
{
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		//LOGE("-----linux.c------libusb_handle_events_timeout_completed----- testTransfer status: %d",testTransfer->status);
