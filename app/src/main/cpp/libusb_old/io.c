	...
	enum libusb_transfer_status status)
{
	...
	if (status == LIBUSB_TRANSFER_COMPLETED
		...
		if (rqlen != itransfer->transferred) {
			LOGD("interpreting short transfer as error");
...
static int handle_events(struct libusb_context *ctx, struct timeval *tv)
{
	...
	if (tv->tv_usec % 1000)
		timeout_ms++;
	//LOGD("-----io.c-----handle_event-------before poll numbFd: %d; timeout: %d \n",nfds,timeout_ms);
	//LOGD("poll() %d fds with timeout in %dms", nfds, timeout_ms);
	r = usbi_poll(fds, nfds, timeout_ms);
	if (r == 0) {
		free(fds);
		LOGD("-----io.c-----handle_event-------r=0 and exit\n");
	...
	if (fds[0].revents) {
		...
		if (r == 1) {
			r = 0;
			LOGD("-----io.c-----handle_event-------r=0 and exit when another thread interrupt event handling\n");
			goto handled;
		} else {
...
#endif
//	LOGD("-----io.c------libusb_handle_events----- numbFd: %d; numbPoll(r):%d",nfds,r);
	r = usbi_backend->handle_events(ctx, fds, nfds, r);
//	LOGD("-----io.c------libusb_handle_events_----- testTransfer status: %d; r: %d",testTransfer->status,r);
	if (r)
